...
export default function EventSelectorPage() {
  ...
  const computed = useMemo((): {
    ...
    if (tab === "signature") {
      ...
      const validation = validateSignature(trimmed, "event");
      if (!validation.valid) {
        return { selector: null, signature: null, error: validation.error! };
      }
      try {
        const selector = generateEventSelector(validation.canonicalSignature!);
        return { selector, signature: validation.canonicalSignature!, error: null };
      } catch {
        return { selector: null, signature: null, error: "Failed to generate event topic hash." };
      }
    } else {
      ...
      if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name)) {
        return { selector: null, signature: null, error: "Invalid event name. Use letters, numbers, and underscores." };
      }
      ...
      try {
        const selector = generateEventSelector(sig);
                ...
                <div>
                  ...
                  {params.length === 0 && (
                    <p style={{ color: "var(--muted)", fontSize: 13, margin: "0 0 8px", fontStyle: "italic" }}>
                      No parameters. Add one below or leave empty for a no-arg event.
                    </p>
                  )}
                  {params.map((p, i) => (
                    <ParamRow
          ...
          <div className="panel-body" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                >
                  {computed.selector || "0x________________________________________________________________"}
                </code>
